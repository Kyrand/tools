...
def format_commit_message(message):
    ...
    linkified = re.sub(
        issue_pattern,
        r'<a href="https://github.com/Kyrand/tools/issues/\1">#\1</a>',
        linkified,
    )
...
<head>
    ...
    <title>tools.kyrandale.com colophon</title>
    <style>
        body {
...
<body>
<nav>
    <p><a href="/">Kyran Dale's Tools</a> <a href="https://kyrandale.com/">My blog</a></p>
</nav>
<section class="body">
    <h1>tools.kyrandale.com colophon</h1>
"""

    # Add the tool count to the existing paragraph
    html_content += f"""
    <p>The tools on <a href="https://tools.kyrandale.com/">tools.kyrandale.com</a> are listed here with their development history ({tool_count} tools total).</p>
    <p>This page lists the commit messages for each tool, many of which link to the conversation or context used to produce the code.</p>
"""

    # Modified tool heading HTML
    for page_name, page_data in sorted_pages:
        tool_url = f"https://tools.kyrandale.com/{page_name.replace('.html', '')}"
        github_url = f"https://github.com/Kyrand/tools/blob/main/{page_name}"
        commits = page_data.get("commits", [])
...
"""
        ...
        for commit in commits:
            ...
            commit_url = f"https://github.com/Kyrand/tools/commit/{commit_hash}"
